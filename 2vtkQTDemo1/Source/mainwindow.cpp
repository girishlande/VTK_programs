...
void MainWindow::on_actionImport_STL_file_triggered() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, "Select STL file", "../models",
      "OBJ files(*.obj);STL files(*.stl);All Files (*.*)");  //"STL Files
                                                             //(*.stl);;
...
void MainWindow::sliderChanged1(int value) {
  if (s1_viewer) {
    ...
    myVtkInteractorStyleImage *
        style = (myVtkInteractorStyleImage*)view1->GetRenderWindow()->GetInteractor()->GetInteractorStyle();
    if (style) {
...
void MainWindow::sliderChanged2(int value) {
  if (s2_viewer) s2_viewer->SetSlice(value);
}
