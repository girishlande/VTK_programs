...
void MainWindow::test1() {
  ...
  cout << "\n Image renderer:\n";
  displyRendererDetails(m_vtkImageViewer->GetRenderer());
  
  ViewportBorder();
  drawAxialLine();

  m_renderer->GetActiveCamera()->Zoom(4);
...
void MainWindow::displyRendererDetails(vtkRenderer* renderer) {
  ...
  cout << "\n Origin and size:" << pOrigin[0] << "  " << pOrigin[1] << "  Size:"
       << pSize[0] << "  " << pSize[1];
}
...
void MainWindow::calculateViewportDetails(vtkImageActor* imageActor) {
  ...
  cout << "\n viewport:" << vminx << "," << vminy << "," << (vminx + vxsize) << "," << (vminy + vysize);
  //m_renderer->SetViewport(0.05, 0.4, 0.6, 0.95);
}