...
MainWindow::MainWindow(QWidget* parent)
  ...
  this->showMaximized();
  this->setWindowTitle(
      "Demo of Topo view in 2X2 layout");
...
void MainWindow::createMultipleViewports(int rows, int cols) {
  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      ...
      std::shared_ptr<avTopoViewer> topo =
          std::make_shared<avTopoViewer>(m_imageVolume, renderer);
      topo->SetViewSize(0.2, 0.2);
      m_topo.push_back(topo);
...
void MainWindow::UpdateImagesInViewports() {
  ...
  for (int i = 0; i < m_renderers.size(); i++) {
    ...
    UpdateSliceNumberCornerText(index, rendererIndex);
    UpdateTopo(index, rendererIndex);
...
void MainWindow::ReadImageVolume() {
  ...
  for (int i = 0; i < D[2]; i++) {
    voi->SetVOI(0, 0, i, D[0], D[1], i);
    voi->Update();
    m_imagedata.push_back(voi->GetOutput());
  }*/
