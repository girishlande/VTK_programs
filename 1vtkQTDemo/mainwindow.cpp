...
MainWindow::MainWindow(QWidget* parent)
    ...
    , s1_viewer(nullptr), s2_viewer(nullptr)
{
    ...
    s1->setFixedWidth(20);
    s1->setFixedHeight(300);
    connect(s1, SIGNAL(valueChanged(int)), this, SLOT(sliderChanged1(int)));

    s2 = new QScrollBar(this);
    s2->setFixedWidth(20);
    s2->setFixedHeight(300);
    connect(s2, SIGNAL(valueChanged(int)), this, SLOT(sliderChanged2(int)));
...
void MainWindow::DoView_1()
{
    ...
    vtkSmartPointer<vtkPolyDataMapper> sm1 = vtkSmartPointer<vtkPolyDataMapper>::New();
...
{
    vtkSmartPointer<vtkSphereSource> s2 = vtkSmartPointer<vtkSphereSource>::New();
    s2->Update();
    vtkSmartPointer<vtkPolyDataMapper> sm2 = vtkSmartPointer<vtkPolyDataMapper>::New();
    sm2->SetInputConnection(s2->GetOutputPort());
    vtkSmartPointer<vtkActor> sa2 = vtkSmartPointer<vtkActor>::New();
    sa2->SetMapper(sm2);
...
{
    std::string inputFilename = "../models/face.stl";
    vtkSmartPointer<vtkSTLReader> reader = vtkSmartPointer<vtkSTLReader>::New();
    reader->SetFileName(inputFilename.c_str());
    reader->Update();
    vtkSmartPointer<vtkPolyDataMapper> sm3 = vtkSmartPointer<vtkPolyDataMapper>::New();
    sm3->SetInputConnection(reader->GetOutputPort());
    vtkSmartPointer<vtkActor> sa3 = vtkSmartPointer<vtkActor>::New();
    sa3->SetMapper(sm3);
    vtkSmartPointer<vtkRenderer> r3 = vtkSmartPointer<vtkRenderer>::New();
    r3->AddActor(sa3);
...
void MainWindow::on_actionImport_STL_file_triggered()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(
        ...
        "Select STL file",
        "../models",
        "OBJ files(*.obj);STL files(*.stl);All Files (*.*)"); //"STL Files (*.stl);;
...
void MainWindow::UpdateViewForDICOM()
{
    ...
    if (current_view_index == 1) {
        ...
        s2_viewer = imageViewer.Get();
        s2->setMinimum(imageViewer->GetSliceMin());
        s2->setMaximum(imageViewer->GetSliceMax());
        s2->setValue(imageViewer->GetSlice());
    }
...
void MainWindow::sliderChanged2(int value)
{
    if (s2_viewer)
        s2_viewer->SetSlice(value);
}
