...
void KeypressCallbackFunction(vtkObject* caller, long unsigned int vtkNotUsed(eventId), void* vtkNotUsed(clientData), void* vtkNotUsed(callData))
{
	...
	if (keysym.compare("Up") == 0) {
		double origin[3];
		double inc[3] = { 0, 0.1, 0 };
		clipPlane1->GetOrigin(origin);
		vtkMath::Add(origin, inc, origin);
		clipPlane1->SetOrigin(origin);
		clipPlane1->Modified();
		iren->GetRenderWindow()->Render();
	} else {
		double origin[3];
		double inc[3] = { 0.0, -0.1, 0 };
		clipPlane1->GetOrigin(origin);
		vtkMath::Add(origin, inc, origin);
		clipPlane1->SetOrigin(origin);
		clipPlane1->Modified();
	...
	if (keysym.compare("Up") == 0) {
		double origin[3];
		double inc[3] = { 0, 0, 0.1 };
		clipPlane2->GetOrigin(origin);
		vtkMath::Add(origin, inc, origin);
		clipPlane2->SetOrigin(origin);
		clipPlane2->Modified();
		iren->GetRenderWindow()->Render();
	} else {
		double origin[3];
		double inc[3] = { 0.0, 0, -0.1 };
		clipPlane2->GetOrigin(origin);
		vtkMath::Add(origin, inc, origin);
		clipPlane2->SetOrigin(origin);
		clipPlane2->Modified();
...
int main(int, char *[])
{
	...
	clipPlane1 =
		vtkSmartPointer<vtkPlane>::New();
	clipPlane1->SetNormal(0, 1.0, 0);
	clipPlane1->SetOrigin(0.0, 0.0, 0.0);

	// Define a clipping plane
	clipPlane2 =
		vtkSmartPointer<vtkPlane>::New();
	clipPlane2->SetNormal(0, 0, 1.0);
	clipPlane2->SetOrigin(0.0, 0.0, 0.0);
	...
	vtkSmartPointer<vtkClipPolyData> clipper1 =
		vtkSmartPointer<vtkClipPolyData>::New();
	clipper1->SetInputConnection(clipper->GetOutputPort());
	...
	superquadricMapper->SetInputConnection(clipper2->GetOutputPort());
