...
void avTopoViewerEx::RemoveActors() {
    ...
    for (auto a : m_scanNumbers) {
        m_renderer->RemoveActor(a.actor);
    }
    m_scanNumbers.clear();
}
...
void avTopoViewerEx::UpdateActorPosition(int dx, int dy) {
    ...
    m_scanLinesActor->SetDisplayPosition(dx, dy);
    UpdateScanLineNumberAnnotation(dx, dy);
}
...
void avTopoViewerEx::AddGridLines() {
    m_scanNumbers.clear();

    std::cout << "\nTopo: Normalised width:" << m_topoWidth << " Height:" << m_topoHeight;
    std::cout << "\nTopo: Device width:" << m_topoWidth_DC << " Height:" << m_topoHeight_DC;

    // Relative width and height to TopoView (in normalized)
    double gridW = 0.8;
    double gridH = 0.8;

    // Normalized width and height of GRID.
    double gridWidth = gridW * m_topoWidth;
    double gridHeight = gridH * m_topoHeight;
    std::cout << "\nGrid: Normalised width:" << gridWidth << " Height:" << gridHeight;

    // Grid width and Height in device space 
    int gridWidthDC = ceil(m_viewportWidthDC * gridWidth);
    int gridHeightDC = ceil(m_viewportHeightDC * gridHeight);
    std::cout << "\nGrid: Device width:" << gridWidthDC << " Height:" << gridHeightDC;

    // relative position from Topo(0,0) in Normalized viewport space
    double gridMinX = ((1.0 - gridW) / 2 ) * m_topoWidth;
    double gridMaxX = (1.0 - ((1.0 - gridW) / 2)) * m_topoWidth;
    double gridMinY = ((1.0 - gridH) / 2) * m_topoHeight;
    double gridMaxY = (1.0 - ((1.0 - gridH) / 2)) * m_topoHeight;

    // relative position from Topo(0,0) in Device space
    int gridMinXDC = round(gridMinX * m_viewportWidthDC);
    int gridMaxXDC = round(gridMaxX * m_viewportWidthDC);
    int gridMinYDC = round(gridMinY * m_viewportHeightDC);
    int gridMaxYDC = round(gridMaxY * m_viewportHeightDC);

    int gapThickness = 2;
    int lineThickness = 2;
    int numLines = floor(gridHeightDC / (gapThickness+ lineThickness));
    gridHeightDC = numLines * (gapThickness + lineThickness);
    gridHeight = (double)gridHeightDC / m_viewportHeightDC;
    
    double lineHeight = gridHeight / numLines;

    numLines = numLines + 1; // consider extra line
    vtkNew<vtkPoints> points;
    vtkIdType numPoints = (vtkIdType)numLines * 2;
    points->SetNumberOfPoints(numPoints);
    for (int i = 0; i < numLines; i++) {
        double height = lineHeight * i;
        vtkIdType pointIndex = 2 * (vtkIdType)i;
        points->InsertPoint(pointIndex, gridMinX, gridMinY + height, 0);
        points->InsertPoint(pointIndex + 1, gridMaxX, gridMinY + height, 0);

        if (i % 5 == 0) {
            AddLineNumber(m_topoX_DC + gridMaxXDC, m_topoY_DC + gridMinYDC + 
                (gapThickness+lineThickness) *i - 6, (i + 1), numLines);
        }
    }
    ...
    vtkNew<vtkActor2D> actor;
    actor->SetMapper(mapper);
    m_scanLinesActor = actor.Get();
    m_scanLinesActor->SetDisplayPosition(m_topoX_DC, m_topoY_DC);
    actor->GetProperty()->SetColor(m_colors->GetColor3d("yellow").GetData());
    actor->GetProperty()->SetLineWidth(1.0);

    m_renderer->AddViewProp(actor);
}

void avTopoViewerEx::AddLineNumber(int x, int y, int index, int total) {
    std::string s = std::to_string(index) + "/" + std::to_string(total);
    ...
    textActor->GetProperty()->SetColor(m_colors->GetColor3d("yellow").GetData());
    m_scanNumbers.push_back(ScanlineAnnotation{ textActor.Get(),(x - m_topoX_DC), (y - m_topoY_DC) });
    m_renderer->AddActor2D(textActor);
}

void avTopoViewerEx::UpdateScanLineNumberAnnotation(int dx, int dy) {
    for (auto a : m_scanNumbers) {
        a.actor->SetDisplayPosition(a.x + dx, a.y + dy);
    }
}
